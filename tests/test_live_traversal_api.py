...
def test_traversal_stream_requires_auth_and_filters_paths(tmp_path: Path) -> None:
    ...
    with TestClient(app) as unauthenticated_client:
        assert unauthenticated_client.get("/api/v1/graph/traversal/stream").status_code == 401
    with TestClient(app, headers={"Authorization": f"Bearer {token}"}):
        route = next(route for route in app.routes if getattr(route, "path", None) == "/api/v1/graph/traversal/stream")
        record = app.state.token_service.verify(token)
        response = route.endpoint(auth=AuthContext(record, AccessPolicy(record.rules, record.admin)))
        ...
        async def collect() -> list[str]:
            try:
                return [await anext(response.body_iterator), await anext(response.body_iterator)]
            finally:
