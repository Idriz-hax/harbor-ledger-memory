...
def _service(tmp_path: Path) -> tuple[ScanService, CatalogSession, object]:
    ...
    settings = Settings(vault_path=tmp_path, database_url=f"sqlite:///{tmp_path / 'catalog.db'}")
...
def test_materialized_facts_are_deterministic_and_weighted(tmp_path: Path) -> None:
    ...
    try:
        ...
        service.full_scan()
        version = session.scalar(select(GraphProjectionVersion).where(GraphProjectionVersion.active.is_(True)))
        assert version is not None
        nodes = session.scalars(select(GraphNodeFact).where(GraphNodeFact.version_id == version.id)).all()
        edges = session.scalars(select(GraphEdgeFact).where(GraphEdgeFact.version_id == version.id)).all()
        assert [node.path for node in nodes] == sorted(node.path for node in nodes)
        assert any(edge.edge_type == "links_to" and edge.weight == 2.0 for edge in edges)
    finally:
        session.close()
        engine.dispose()


def test_materialization_swaps_and_failure_rolls_back(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    service, session, engine = _service(tmp_path)
    try:
        note = tmp_path / "AI" / "note.md"
        note.write_text("# One\n", encoding="utf-8")
        service.full_scan()
        first = session.scalar(select(GraphProjectionVersion).where(GraphProjectionVersion.active.is_(True)))
        assert first is not None
        note.write_text("# Two\n", encoding="utf-8")
        service.full_scan()
        versions = session.scalars(select(GraphProjectionVersion).order_by(GraphProjectionVersion.created_at)).all()
        ...
        with pytest.raises(RuntimeError, match="materialization failed"):
            service.full_scan()
        active = session.scalar(select(GraphProjectionVersion).where(GraphProjectionVersion.active.is_(True)))
...
def test_snapshot_handle_creation_cleans_expired_and_caps_capacity(tmp_path: Path) -> None:
    service, session, engine = _service(tmp_path)
    try:
        ...
        service.full_scan()
        session.add(GraphSnapshotHandle(
            handle="expired", version_id=session.scalar(select(GraphProjectionVersion).where(GraphProjectionVersion.active.is_(True))).id,
            policy_fingerprint="public", scope_fingerprint="scope", level=0, offset=1,
        ...
        cursor = None
        for _ in range(12):
            view = GraphProjectionService(session).view(1, page_size=1, cursor=cursor, policy=policy, policy_fingerprint=policy.fingerprint())
