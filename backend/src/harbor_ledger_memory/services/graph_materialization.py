...
def materialize_graph(session: Session, *, created_at: str | None = None) -> str:
        ...
        node_rows.append(
            {
                ...
                "top_folder": PurePosixPath(path).parts[0]
    ...
    for source, target, data in sorted(
        ...
        key=lambda item: (item[0], item[1], str(item[2].get("edge_type", ""))),
    ):
    ...
    payload = json.dumps(
        {"nodes": node_rows, "edges": edge_rows}, sort_keys=True, separators=(",", ":")
    )
    ...
    session.execute(
        ...
        .where(GraphProjectionVersion.active.is_(True))
        .values(active=False)
    )
    ...
    pinned = session.scalars(
        ...
        .order_by(
            GraphSnapshotHandle.expires_at.desc(), GraphSnapshotHandle.handle.asc()
        )
    ...
    if keep_handles:
        session.execute(
            delete(GraphSnapshotHandle).where(
                ~GraphSnapshotHandle.handle.in_(keep_handles)
            )
        )
    else:
    ...
    inactive = session.scalars(
        ...
        .order_by(
            GraphProjectionVersion.created_at.desc(), GraphProjectionVersion.id.desc()
        )
    ).all()
    retained: set[str] = {inactive[0].id} if inactive else set()
