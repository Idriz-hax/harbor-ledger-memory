...
class AccessPolicy:
    ...
    def fingerprint(self) -> str:
        ...
        payload.sort()
        return hashlib.sha256(json.dumps([self._admin, payload], separators=(",", ":")).encode()).hexdigest()

    def readable_clause(self, column: object) -> object:
        """Return a parameterized SQL predicate equivalent to deepest-prefix reads."""
        from sqlalchemy import and_, not_, or_
        predicates = []
        for rule in self._rules:
            prefix = rule.path.as_posix().rstrip("/")
            match = True if prefix == "." else or_(column == prefix, column.like(_like_prefix(prefix), escape="\\"))
            deeper = []
            for other in self._rules:
                if len(other.path.parts) > len(rule.path.parts) and _matching_depth(rule.path, other.path) is not None:
                    oprefix = other.path.as_posix().rstrip("/")
                    deeper.append(True if oprefix == "." else or_(column == oprefix, column.like(_like_prefix(oprefix), escape="\\")))
        ...
        for rule in self._rules:
            if not rule.access.is_readable:
                prefix = rule.path.as_posix().rstrip("/")
                denied.append(True if prefix == "." else or_(column == prefix, column.like(_like_prefix(prefix), escape="\\")))
